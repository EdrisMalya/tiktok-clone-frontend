...
<script>
    ...
    export default {
        components: { UploadError, UploadLayout },
        data(){
            return {
                file: null,
                fileDisplay: false,
        ...
        methods: {
            onDrop(event) {
                ...
                this.fileData = event.dataTransfer.files[0]
                let extension = this.file.name.substring(this.file.name.lastIndexOf('.') + 1)
                if (extension !== 'mp4') {
                    this.errorType = 'file'
                    return
                }
                this.fileDisplay = URL.createObjectURL(event.dataTransfer.files[0])
            },
            onChange(event){
                this.errorType = ''
                ...
                this.fileDisplay = URL.createObjectURL(event.target.files[0])
            },
            discard(){
                this.file = null
